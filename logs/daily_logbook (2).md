...
**Work Done:** Ran three small experiments (not full implementation, just exploration):
...
3. Ran spaCy's NER on a short resume text sample — it detected the candidate's name reasonably well using the `PERSON` label, though it also tagged "Curriculum Vitae" as a name at one point.
